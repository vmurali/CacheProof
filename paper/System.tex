...
structure, with the root connected to the main memory and each leaf
interfaced with a processor (Figure~\ref{hier}). In the following,
$\leaf(c)$ indicates that a cache $c$ is a leaf, and $\parent(c,p)$
indicates that $p$ is the parent of $c$.

Input and output happens at the leaves in the form of a stream of
memory requests issued from and responses issued to the associated
processor. These will be represented in Coq as two mappings each with
integer ($c.\pos$) indicating how many elements in the stream we have
handled so far.
