...
We have chosen as our concrete example is a hierarchical MSI
protocol~\cite{MSI}. Logically, the system is organized as in a tree-like
...
that a cache is a leaf, and $\parent(c,p)$ indicates that $p$ is the parent of
$c$.
...
in the modified state (\Mo) representing that it may be both read and modified,
in the shared state (\Sh) in which it may only be read, or in the Invalid state
(\In), indicating that it can be neither modified or read. This forms a natural
...
processed only if it is in the \Mo{} state. For a non-leaf cache, it denotes
that no cache in the subtree rooted at that cache has more permissions (we are
modeling inclusive caches).  If a cache does not have a particular location,
the it is equivalent to its permission being \In.  Initially, all addresses in
each cache is in the \In{} state, save for those in the root cache which are in
the \Mo{} state.

%Of course, all these invariants have to be enforced by the protocol.


Non-leaf caches also keep a (possibly stale) copy of the permission state of
...
$p.\dstate[c][a]$. The directory entry for a cache, for an address is never
less than the actual cache's permission state for that address.
...
$\textit{ch}[c]$ for the \textit{ch} channel associated with cache $c$).
...
\False{} initially for each of the caches, and for each of the addresses,
because no cache is waiting for any response from either its child or its
parent initially.

The behavior of the system is described via the atomic state transitions. These
...
transition happens. In the actual implementation, several of these transitions
or \emph{guarded atomic actions} happen concurrently in a single hardware
clock, but they will logically happen happen one at a time. Figure~\ref{trans}
gives the atomic transitions for the MIS protocol.
The actions in the transitions in Figure \ref{trans} show only the specific
states that change, the rest of the states remain as is. The notation ($x
...
\emph{compatible} with the upgrade request, and parent's permission state has
...
ParentRecvReq transition of Figure \ref{trans} is defined as follows:
\begin{multline*}
...
in downgrading a cache's directory entry for one of its chilren $c$. The
difference is in the guard for ChildRecvReq transition: the directory entry for
each of the $c$'s children should not be greater than the requested
...
because of the presence of \emph{voluntary} responses. This models eviction of
a location from a cache to make room for a new location.  This eviction usually
...
place only if the cache is not waiting for a response from its parent for
...
Figure~\ref{procside} shows the transitions that serve a processors request in
...
request in our memory subsystem model; in a real system, the data in the cache
for the requested address is sent back to the processor. Our definition of
store atomicity requires responses for both load and store requests, and the
response field contains the $(\textit{Processor}*\mathbb{N})$ pair identifying
the request corresponding to the response, the data from the cache in case of a
load response, and a value for the field \tme. A \Response{} is defined
whenever LoadReq or StoreReq transition takes place. Let $c$ be a leaf cache,
and $t$ be the position of a LoadReq$(c)$ or StoreReq$(c)$ transition in the
ordered list of transitions of the full system starting from the initial state.
The fields of the response are as follows:
\begin{multline*}
