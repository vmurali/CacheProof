...
%\begin{enumerate}
%\item Permissions can be Nothing, Read (only), Read and Write, \ie $\{\}, {R}, {R, W}$
%\item To upgrade = to gain more permissions
%\item To downgrade = to lose permissions
%\item If the child cache wants to gain some permissions for an address, it can
%do so only after the parent directory has authorized it
%\end{enumerate}
%
%\begin{enumerate}
%\item Permissions that a child cache has for an address is a subset of the
%permissions that the parent directory believes the child has for that address
%\item If the parent directory wants a child cache to lose some permissions for
%an address, there is a message that the directory can send notifying the child
%of the directory's intentions (this message is called a downgrade request)
%\item If the cache wants to gain some permissions for an address, there is a
%message that the child can send notifying the directory of the child's
%intentions (this message is called an upgrade request)
%\item If a node wants to send a message to some node, then it can send such the
%message eventually (\ie it can never be blocked from sending the message
%forever)
%\item If the parent directory has sent a downgrade request for an address $a$
%to a child cache $c$, the directory will eventually get a message from $c$
%notifying the directory that the cache has given up the required permissions
%for address $a$ in $c$
%\item If the child cache has sent an upgrade request for an address $a$ to
%parent directory, the child will eventually get a message from the directory
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.53]{network}
