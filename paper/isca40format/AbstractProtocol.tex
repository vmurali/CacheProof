...
%and for each address (\ie \qTwo{ReqFromC}{c}{a}, \qOne{ReqFromPQ}{a},
%\qTwo{RespFromCQ}{c}{a}, \qTwo{ReqToCQ}{c}{a}, \qOne{RespToPQ}{a}, \etc). An
%implementation of a protocol using our methodology does not have to keep a
%separate access queue for each source and each address, but it must adhere to
%the blocking restrictions in Figure~\ref{list:messageOrder}.  These
%restrictions are necessary to avoid deadlocks. The flexibility of the abstract
%protocol allows one to design a wide range of blocking policies.
...
We now describe the how a node handles requests and responses.
...
\begin{algorithmic}
\Rule{handlePReq}{a}
   \State let mesg \two{ReqFromP}{a}{x} exist in \qOne{ReqFromPQ}{a}
...
\begin{algorithmic}
\Rule{handlePResp}{$a$}
   \State let mesg \two{RespFromP}{a}{x} exist in \qOne{RespFromPQ}{a}
   \State remove$(\two{RespFromP}{a}{x})$
   \If {$state(a) \neq \Inv$}
      \State let mesg \two{DataFromP}{a}{d} exist in \qOne{DataFromPQ}{a}
...
\begin{figure}
\begin{algorithmic}
\Rule{handleCResp}{$c, a$}
   \State let mesg \three{RespFromC}{c}{a}{x} exist in \qTwo{RespFromCQ}{c}{a}
   \State remove$(\three{RespFromC}{c}{a}{x})$
   \If {dataXfer$(dir(c,a), x)$}
      \State let mesg \three{DataFromC}{c}{a}{d} exist in \qTwo{DataFromPQ}{c}{a}
...
%Figure~\ref{alg:handleCoreReq} shows the handler for a request from the core,
%which is very similar to a combination of \texttt{handleCReq} and
%\texttt{handleEvict}. The code shows that the cache line is sent back to the
%core for any request, which is not necessary for a store request in an actual
%implementation.
%
%\begin{figure}
%\begin{algorithmic}
%\Rule{handleCoreReq}{\three{ReqFromCore}{a}{x}}
%  \If {$isPresent(a)$}
%     \If {$!isPWait(a)$}
%        \If {$state(a) \ge x$}
%           \State remove$(\three{ReqFromCore}{a}{x})$
%           \State send$(\three{RespToCore}{a}{x}{data(a)})$
%        \ElsIf {$state(a) < x$}
%              \State $isPWait(a) \assigns \True$
%              \State send$(\two{ReqToP}{a}{x})$
%        \EndIf
...
%           \State send($\two{RespToP}{a'}{Inv}$)
%           \If {dataXfer($state(a'), Inv$)}
%              \State send($\two{DataToP}{a'}{Inv}$)
%           \EndIf
%        \EndIf
%        \State $isPresesnt(a') \assigns \False$
%        \State allocLine($a$)
%        \State $isPWait(a) \assigns \True$
%        \State send$(\two{ReqToP}{a}{x})$
%     \EndIf
...
%\EndRule
%\end{algorithmic}
%\caption{Handle request from core}
%\label{alg:handleCoreReq}
%\end{figure}
