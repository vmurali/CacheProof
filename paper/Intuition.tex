\section{Proof of Store Atomicity for the MSI Protocol}
\label{Sec:Safety}
...
REFERENCE HERE}.
\adamc{I presume we'll want to submit a tarball of the Coq source code with the paper.
...
\adamc{Something needs to be fixed up here, for consistency in how many invariants there are.}

Given these invariants, we codify our intuition by showing that
Theorem~\fullref{storeAtomicity} follows naturally from the
...
\subsection{Reducing to Pairwise Invariants}
\label{pairwise}
...
of \fullref{localCompatibility} imply a $\ge$ relation between the
...
\adamc{This section seems like a prime place to reduce detail to save space, while also saving readers from trying to understand proofs that are mechanized, anyway.}
...
Invariant \fullref{conservative} -- we prove it by considering the last message
