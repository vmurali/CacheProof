...
informal for brevity. As mentioned earlier, all the proofs can be found at~\xxx{MURALI PUT
...
Invariants~\fullref{conservative} and \fullref{localCompatibility1} define a
...
lowest common ancestor $x$ that is an ancestor of both $c_1$ and $c_2$. $x$ has
two distinct children $x_1$ and $x_2$ such that they are the ancestors of $c_1$
and $c_2$ respectively. For any address $a$, $x_1.\state[a]$ and
$x_2.\state[a]$ are compatible by Invariant~\fullref{localCompatibility2}. It
follows that $x_1.\state[a]$ and $x_2.\state[a]$ are compatible since these
values are smaller than the respective directory values. From the previous
paragraph about the relation between the states of a cache and its ancestor, it
follows that $c_1$ and $c_2$ are also compatible. This proves
Invariant~\fullref{nonAncestorCompatible}.

Using the properties discussed in the last two paragraphs, it is easy to prove
Invariant~\ref{latestValue}. The proof is by induction on time $t$. We consider
the cases when a node is either \clean{} in the previous state or not. One can
easily prove that if it was \clean{} in the previous state, then no cache, save
itself, can process a store request, proving the invariant.  If the cache was
not \clean{} in the previous state, it will receive data from a \clean{} cache
and no store requests can be processed when the data is in transit (by
Invariant~\ref{noTransitWrite}). Therefore, by the induction hypothesis it has
the up-to-date data.
...
properties of the MSI transistions of Figure~\ref{trans}, though the argument
is straightforward. Analyzing a parent-child pair will suffice in proving
Invariants~\fullref{localCompatibility1} and \fullref{localCompatibility2}.
Proof of Invariant~\fullref{noTransitWrite} is similar to the proof of
Invariant~\fullref{nonAncestorCompatible}.
