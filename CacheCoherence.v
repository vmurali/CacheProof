    ...
    Section message.
      ...
      Lemma childRecvNoParentRecv {tp tc}:
        ...
        forall {tr}, tr <= tc -> forall {m}, (recvm c m tr /\ fst m <= state c tr) ->
          (forall t, S tr <= t <= tc -> forall m', ~ (recvm c m' t /\ fst m' <= state c t)) ->
          forall {ts}, sendm p m ts -> (forall t', S ts <= t' <= tp -> forall m'', ~ recvm p m'' t') ->
        ~ (nextState c tc > nextState p tp).
        unfold not.
        intros.
        assert (nextState c tc <= nextState c tr) by (apply (noRecvChild2 H1 H3)).
      ...
      Proof.
        ...
        assert (noCRecv: forall y, S tcmax <= y <= tcmin -> forall m, ~ (recvm c m y /\ fst m <= state c y)) by
