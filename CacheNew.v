  ...
  Section CommonBeh.
    ...
    Record CommonBehavior :=
      {
        ...
        procmChange: forall {t a m}, proc mch dst src a t m -> st a (S t) = to m;
        ...
        noSendmProcm: forall {t a m}, mark mch src dst a t m ->
    ...
    Lemma nochange': forall {a t t'},
                       (forall tn, t <= tn < t + t' ->
                                   ...
                                   (forall m, ~ proc mch dst src a tn m)) ->
                       st a t = st a (t + t').
    Proof.
      intros a t t'.
      induction t'.
...
(*    Axiom procrSendm: forall {r}, proc rch p c a t r -> state c a t > to r -> exists {m}, mark mch c p a t m.*)
    ...
    Definition twoPReqEqNeedsPResp :=
      forall a t t1 r1,
    ...
    Section ForA.
      Context {a: Addr}.
      Axiom pRespReq: twoEqPRespFalse ->
                      twoPReqEqNeedsPResp ->
                      forall {t1 t2 t3},
                      forall {m},
                        mark mch p c a t1 m ->
      ...
      Axiom pReqResp: twoEqPRespFalse ->
                      twoPReqEqNeedsPResp ->
                      forall {t1 t2 t3},
                      forall {r},
                        mark rch p c a t1 r ->
...
Module Final (dt: DataTypes) (ch: ChannelPerAddr dt) (st: Stuff dt ch).
  ...
  Section Pair.
    Context {p c: Cache}.
    Variable isParent: parent c = p.
    Definition st := @st.st p c.
    Definition dt := @st.dt p c.


      Lemma whenChildHighProcm: forall {a t},
                                  state c a (S t) > state c a t ->
                                  exists m, proc mch p c a t m /\ to m = state c a (S t).
      Proof.
        intros a t sStgst.
        assert (sStnst: state c a (S t) <> state c a t) by omega.
        pose proof (change st sStnst) as [[m sendmm] | [m procmm] ].
        pose proof (sendmImpSt sendmm) as stgm.
        pose proof (sendmChange st sendmm) as sStem.
        omega.
        exists m.
        intuition.
        pose proof (procmChange st procmm) as sStem.
        intuition.
      Qed.
    ...
    Proof.
      ...
      pose proof (procmChange dt procm) as sth.
      pose proof (sendmImpSt sendm) as sth2.
      pose proof (procmCond procm) as sth3.
    ...
    Proof.
      ...
      pose proof (change dt nestuff) as [sendm | procm].
      firstorder.
    ...
    Proof.
      ...
      firstorder.
      pose proof (procmChange dt procm).
      omega.
    Qed.
    ...
    Lemma cReqPRespCrossInd:
      forall {a t tc tp},
        ...
        forall {r m}, mark rch c p a tc r ->
                      mark mch p c a tp m ->
                      (forall tc', tc' < tc -> ~ proc mch p c a tc' m) ->
    ...
    Proof.
      ...
      pose proof (sendmImpProcr msendm) as [r' rprocr'].
      ...
      pose proof (whenChildHigh t''GtTc tcLtT'') as [t''' [[tcLeT''' t'''LtT''] [m' [mprocm' _]]]].
      ...
      pose proof (whenChildHigh tmurGtT' stt'LtstTc) as [t'' [[tcLeT'' t''LtT'] [m' [mprocm' _]]]].
      pose proof (procImpMark mprocm') as [td [tdLeT'' msendm']].
      assert (tdLet: td <= t) by omega.
    ...
    Lemma pRespFifo: forall {a ts1 ts2 tr2 m1 m2},
      ...
      pose proof (sendmImpProcr markm2) as [r2 procr2].
    ...
    Lemma noTwoPResp: @twoEqPRespFalse p c.
    Proof.
      intros a tx t1 m1 t1LeTx sendm1 t2 m2 t2LeTx sendm2 noprocm1 noprocm2.
      pose proof (sendmImpProcr sendm1) as [r1 procr1].
      pose proof (sendmImpProcr sendm2) as [r2 procr2].
      ...
      assert (noProc: ~ exists t, t3 <= t < t4 /\ exists m, proc mch p c a t m) by (
                                                                                    ...
                                                                                    destruct opts as [t5EqT1 | [t5LtT1 | t5GtT1]]; [
                                                                                      ...
                                                                                      pose proof (sendmImpProcr sendm) as [r procr];
      ...
      pose proof (whenChildHighConv t3LeT5 noProc') as stContra.
      omega.
    ...
    Proof.
      ...
      pose proof (sendmImpProcr msendmp) as [r rprocr].
      pose proof (procImpMark rprocr) as [t' [t'Le0 rsendr]].
      ...
      assert (dirEqSt: dir p c a (S ts) = state c a (S t')) by (
                                                                ...
                                                                pose proof (procmChange dt mprocm) as two;
                                                                  congruence).
      ...
      assert (dirEqSt: state c a tm = dir p c a tm) by (
                                                        pose proof (sendmChange dt msendm) as one; pose proof (procmChange st mprocm) as two;
                                                         congruence).
      omega.
      ...
      assert (contra1: ~ state c a tm > state c a t') by (
                                                          ...
                                                          generalize (whenChildHigh tmGtTs contra) noProc; clear; firstorder).
      assert (cont: state c a tm <= state c a t') by omega.
      pose proof (sendrImpSt st rsendr) as toRGtDir.
      pose proof (sendmImpProcrGe msendm rprocr) as toMGeToR.
      ...
      pose proof (procrCond rprocr) as fromRLe.
      ...
      pose proof (sendmImpSt msendm) as sGtTom.
      pose proof (procmChange dt mprocm0) as dSEqTom0.
      pose proof (sendmChange st msendm0) as sSEqTom0.
      pose proof (procrCond rprocr) as dLeFromr.
      ...
      pose proof (sendmImpProcr sendm) as [r1 procr1].
      ...
      clear stcLet1 gtRel.
      pose proof (voluntary rsendr t1LtTc msendmc stcLet2) as [r1 [procr1 sTcGtToR1]].
      ...
      pose proof (sendrImpNoSendm t2LtTp sendr1 msendmp) as [t' [[t2LtT' t'LtTp] [m [procm toMGeToR1]]]].
      pose proof (procImpMark procm) as [t'' [t''LeT' sendm]].
      ...
      rewrite St''EqTc in *.
      omega.
      pose proof (whenChildHigh St''LtTc stTcGtStST'') as [ts [cond [s [procs _]]]].
      ...
      rewrite tsrEqT' in *.
      apply (noSendmProcm dt sends procm).
      ...
      assert (notst2Gtst1: ~ state c a t2 > state c a (S t1)) by (
                                                                  ...
                                                                  destruct eqOrNot as [eq| St1LtT2]; [
                                                                    ...
                                                                    unfold not; intros st; pose proof (whenChildHigh St1LtT2 st) as some; firstorder]).
      assert (stt2LestT1: state c a t2 <= state c a (S t1)) by omega.
      pose proof (procmCond rprocr) as fromRLe.
      ...
      pose proof (procmChange dt mprocm0) as dSEqTom0.
      pose proof (sendmChange st msendm0) as sSEqTom0.
      pose proof (procmCond rprocr) as dLeFromr.
    ...
    Proof.
      ...
      assert (noCProc: forall tm, t' <= tm < t2 -> forall m', ~ proc mch p c a tm m') by (
                                                                                          ...
                                                                                          destruct opts as [tsLtT | [tsEqT | [tLtTsLtT1  | t1LeTs ]]]; [
                                                                                            ...
                                                                                            rewrite tsEqT in *;
                                                                                              apply (noSendmProcm dt sendm' procm) |
      ...
      pose proof (procmChange dt procm) as st2.
      omega.
      ...
      pose proof (procmChange st procm) as st1.
    ...
    Proof.
      ...
      pose proof (sendrImpNoSendm tsLtTn sendr3 sendm) as [t' [cond3 [m' [procm' toM'LeToR3]]]].
      ...
      rewrite t''EqTy in *.
      apply (noSendmProcm st sendm procq).
      ...
      pose proof (procmChange dt procm) as eq2.
      pose proof (sendmImpSt sendm) as eq3.
      pose proof (sendmFrom st sendm) as eq4.
      pose proof (procmCond procm) as eq5.
      omega.
    ...
    Proof.
      ...
      pose proof (procmChange dt procm) as eq1.
    ...
    Proof.
      ...
      assert (t2_ne_tm: t2 = tm -> False) by (intro r; rewrite r in *; apply (noSendmProcm st markm_mk procm_mm)).
      ...
      pose proof (procmChange st procm_mm) as toM.
      ...
      pose proof (procmChange dt procmm) as toM.
